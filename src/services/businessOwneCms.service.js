...
exports.cmsListService = async (userId) => {
    const cms = await CMS.find({ updated_by: userId, updatedByModel: 'User' }).populate('updated_by', 'name');
...
exports.cmsGetService = async (id, userId) => {
    const cms = await CMS.findOne({ _id: id, updated_by: userId, updatedByModel: 'User' }).populate('updated_by', 'name');
