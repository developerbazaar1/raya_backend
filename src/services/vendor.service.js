...
exports.vendorListService = async (userId) => {
    const vendors = await Vendor.find({ businessOwnerId: userId }).select('companyName representativeName email phoneNumber role notes');
...
exports.vendorDetailsService = async (vendorId, userId) => {
    ...
    const vendor = await Vendor.findOne({ _id: vendorId, businessOwnerId: userId }).select('companyName representativeName email phoneNumber role notes createdAt businessOwnerId');
    if (!vendor) {
