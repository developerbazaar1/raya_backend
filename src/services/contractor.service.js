...
exports.contractorListService = async (businessOwnerId) => {
    const contractors = await Contractor.find({ businessOwnerId }).select('companyName contractorName email phoneNumber role notes');
...
exports.contractorDetailsService = async (contractorId, businessOwnerId) => {
    ...
    const contractor = await Contractor.findOne({ _id: contractorId, businessOwnerId }).select('companyName contractorName email phoneNumber role createdAt businessOwnerId');
    if (!contractor) {
...
exports.updateContractorService = async (contractorId, body, userId) => {
    ...
    // if (email) {
    //     const existingContractor = await Contractor.findOne({ email, _id: { $ne: contractorId } });
    //     if (existingContractor) {
