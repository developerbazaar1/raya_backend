const { createTimeOffRequest, getTimeOffRequest, updateNewChangeOffRequest } = require('../../services/businessOwnerTeam/timeOff.service');
...
exports.updateNewChangeOffRequest = async (req, res) => {
    const data = await updateNewChangeOffRequest(req.body, req.user.userId);
    res.status(200).json({
        status: 'success',
